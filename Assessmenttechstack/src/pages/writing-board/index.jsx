...
function WritingBoard() {
  ...
  const handleLike = (idea) => {
    dispatch(likeWritingResource({ id: idea.id, currentLikes: idea.likes }));
  };
  ...
  return (
    <div className="min-h-screen p-6 bg-slate-50">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-3xl">Writing Board</h1>
      ...
      {status === "loading" && (
        <div className="mb-4 p-3 rounded bg-white border">Loading writing ideas from MongoDB…</div>
      )}
      {error && (
          ...
          <tbody>
            {writingIdeas.map((idea) => (
              <tr key={idea.id}>
                ...
                <td className="border px-4 py-2">{idea.ownerName || ""}</td>
                <td className="border px-4 py-2 text-center">
                  <button
