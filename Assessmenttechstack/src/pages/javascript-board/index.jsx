// src/pages/javascript-board/JavaScriptBoard.jsx
import React, { useState, useEffect, useContext, useMemo } from "react";
import { useSelector, useDispatch } from "react-redux";
import { useNavigate } from "react-router-dom";
import {
  fetchJavascriptResources,
  createJavascriptResource,
  likeJavascriptResource
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-card p-6 rounded shadow-lg w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">Add New JavaScript Idea</h2>
        <form onSubmit={onSubmit}>
          ...
          <input
            placeholder="Optional Link (e.g., GitHub repo, CodePen)"
            value={newIdea.link}
...
function JavaScriptBoard() {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { userEmail } = useContext(AuthContext);

  const board = useSelector((state) => state.javascriptBoard);
  const resources = board?.javascriptResources || [];
  ...
  useEffect(() => {
    dispatch(fetchJavascriptResources());
  }, [dispatch]);
  ...
  const handleLike = (idea) => {
    dispatch(likeJavascriptResource({ 
      id: idea.id, 
    ...
    <div className="min-h-screen p-4 bg-background">
      ...
      {status === "loading" && (
        <div className="mb-6 p-4 rounded-lg bg-card border border-border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-muted-foreground">Loading JavaScript resources from MongoDB…</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">
          ...
          <button 
            onClick={() => dispatch(fetchJavascriptResources())}
      ...
      {resources.length === 0 && status !== "loading" ? (
        <div className="bg-card rounded-lg shadow p-8 text-center">
          <p className="text-muted-foreground text-lg">No JavaScript ideas yet. Be the first to share!</p>
          <button
            onClick={() => setShowModal(true)}
...
export default JavaScriptBoard;
