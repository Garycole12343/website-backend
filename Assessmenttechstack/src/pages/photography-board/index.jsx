// src/pages/photography-board/PhotographyBoard.jsx
import React, { useState, useEffect, useContext, useMemo } from "react";
import { useSelector, useDispatch } from "react-redux";
import { useNavigate } from "react-router-dom";
import {
  fetchPhotographyResources,
  createPhotographyResource,
  likePhotographyResource
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-card p-6 rounded shadow-lg w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">Add New Photography Idea</h2>
        <form onSubmit={onSubmit}>
          ...
          <input
            placeholder="Optional Link (e.g., portfolio URL)"
            value={newIdea.link}
...
function PhotographyBoard() {
  ...
  const board = useSelector((state) => state.photographyBoard);
  const resources = board?.photographyResources || [];
  ...
  useEffect(() => {
    ...
    console.log("Photography Resources:", resources);
  }, [currentUserEmail, resources]);

  useEffect(() => {
    dispatch(fetchPhotographyResources());
  }, [dispatch]);
  ...
  const handleLike = (idea) => {
    dispatch(likePhotographyResource({ 
      id: idea.id, 
  ...
  return (
    <div className="min-h-screen p-4 bg-background">
      <div className="flex flex-col sm:flex-row items-center justify-between mb-6 gap-4">
        <h1 className="text-3xl font-bold text-foreground">Photography Board</h1>
        
        <button
          onClick={() => setShowModal(true)}
          className="bg-yellow-600 hover:bg-yellow-700 text-white px-5 py-2.5 rounded-lg font-medium transition-colors"
          aria-label="Add new photography idea"
        >
          + Add Photography Idea
        </button>
      </div>
      ...
      {status === "loading" && (
        <div className="mb-6 p-4 rounded-lg bg-card border border-border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-muted-foreground">Loading photography ideas from MongoDB…</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">
          ...
          <button 
            onClick={() => dispatch(fetchPhotographyResources())}
      ...
      {resources.length === 0 && status !== "loading" ? (
        <div className="bg-card rounded-lg shadow p-8 text-center">
          <p className="text-muted-foreground text-lg">No photography ideas yet. Be the first to share!</p>
          <button
            onClick={() => setShowModal(true)}
            className="mt-4 bg-yellow-600 hover:bg-yellow-700 text-white px-5 py-2.5 rounded-lg font-medium"
          >
            Add First Idea
          </button>
        </div>
                      ...
                      <tr key={idea.id} className="hover:bg-background">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            ...
                            className="bg-yellow-50 hover:bg-yellow-100 text-yellow-700 px-3 py-1.5 rounded-lg font-medium border border-yellow-200 transition-colors"
...
export default PhotographyBoard;
