...
function CookingBoard() {
  ...
  const handleLike = (idea) => {
    dispatch(likeCookingResource({ id: idea.id, currentLikes: idea.likes }));
  };
  ...
  return (
    <div className="min-h-screen p-4 bg-slate-50">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-3xl">Cooking Board</h1>
      ...
      {status === "loading" && (
        <div className="mb-4 p-3 rounded bg-white border">
          Loading cooking ideas from MongoDB…
        </div>
      )}
      {error && (
          ...
          <tbody>
            {resources.map((idea) => (
              <tr key={idea.id} className="border-t">
                ...
                <td className="p-2">
                  {idea.link && (
                    <a href={idea.link} target="_blank" rel="noreferrer" className="text-green-600">
                      Visit
                    </a>
                  )}
                </td>
