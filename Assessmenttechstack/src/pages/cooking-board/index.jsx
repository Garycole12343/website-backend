...
import {
  fetchCookingResources,
  createCookingResource,
  likeCookingResource
} from "../../store/slices/cookingBoardSlice";
      ...
      <div className="bg-card p-6 rounded w-full max-w-md">
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            ...
            className={`bg-green-600 text-white px-4 py-2 rounded ${isLoading || !message.trim() || !hasValidEmail ? 'opacity-50 cursor-not-allowed' : 'hover:bg-green-700'}`}
          >
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-card p-6 rounded shadow-lg w-full max-w-md">
        ...
        <form onSubmit={onSubmit}>
          ...
          <input
            placeholder="Optional Link (e.g., recipe URL, video)"
            value={newIdea.link}
...
function CookingBoard() {
  ...
  useEffect(() => {
    ...
    console.log("Cooking Resources:", resources);
  }, [currentUserEmail, resources]);

  useEffect(() => {
    dispatch(fetchCookingResources());
  }, [dispatch]);
    ...
    <div className="min-h-screen p-4 bg-background">
      ...
      {status === "loading" && (
        <div className="mb-6 p-4 rounded-lg bg-card border border-border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-muted-foreground">Loading cooking ideas from MongoDB…</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">
          ...
          <button 
            onClick={() => dispatch(fetchCookingResources())}
      ...
      {resources.length === 0 && status !== "loading" ? (
        <div className="bg-card rounded-lg shadow p-8 text-center">
          <p className="text-muted-foreground text-lg">No cooking ideas yet. Be the first to share!</p>
          <button
            onClick={() => setShowModal(true)}
                      ...
                      <tr key={idea.id} className="hover:bg-background">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            ...
                            className="bg-green-50 hover:bg-green-100 text-green-700 px-3 py-1.5 rounded-lg font-medium border border-green-200 transition-colors"
