  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-card p-6 rounded shadow-lg w-full max-w-md">
        <h2 className="text-xl font-bold mb-4">Add New Art Idea</h2>
...
function ArtBoard() {
  ...
  const resources = board?.artResources || [];
  ...
  useEffect(() => {
    ...
    console.log("Art Resources:", resources);
  }, [currentUserEmail, resources]);

  useEffect(() => {
    dispatch(fetchArtResources());
  }, [dispatch]);
    ...
    <div className="min-h-screen p-4 bg-background">
      ...
      {status === "loading" && (
        <div className="mb-6 p-4 rounded-lg bg-card border border-border shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <span className="text-muted-foreground">Loading art resources from MongoDB…</span>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="mb-6 p-4 rounded-lg bg-red-50 border border-red-200 text-red-700">
          ...
          <button 
            onClick={() => dispatch(fetchArtResources())}
      ...
      {resources.length === 0 && status !== "loading" ? (
        <div className="bg-card rounded-lg shadow p-8 text-center">
          <p className="text-muted-foreground text-lg">No art resources yet. Be the first to share!</p>
          <button
            onClick={() => setShowModal(true)}
